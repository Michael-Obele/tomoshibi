<script lang="ts">
  import {
    ...
    searchWeb,
    getCrawlStatus,
  } from "$lib/remote/tomoshibi.remote";
  ...
  const crawlOptions = new PersistedState("tomoshibi-crawl-options", {
    mode: "smart",
    maxDepth: 2,
    limit: 10,
    screenshot: false,
  ...
  const searchOptions = new PersistedState("tomoshibi-search-options", {
    mode: "default",
    category: "",
    rerank: false,
    limit: 5,
  ...
  const mapOptions = new PersistedState("tomoshibi-map-options", {
    search: "",
    limit: 100,
  });

  const monitorOptions = new PersistedState("tomoshibi-monitor-options", {
  ...
  let activeTab = $derived(
    (ALL_TABS as readonly string[]).includes(
      ...
      {#each searchHistory as item (item.id)}
        ...
        >
          <div class="mb-1.5 flex items-center gap-2">
            {#if item.type === "scrape"}
              <div
                class="flex size-5 items-center justify-center rounded bg-blue-500/10"
              >
                <Globe class="size-3 text-blue-500" />
              </div>
            {:else if item.type === "crawl"}
              <div
                class="flex size-5 items-center justify-center rounded bg-amber-500/10"
              >
                <Layers class="size-3 text-amber-500" />
              </div>
            {:else if item.type === "map"}
              <div
                class="flex size-5 items-center justify-center rounded bg-emerald-500/10"
              >
                <Map class="size-3 text-emerald-500" />
              </div>
            {:else if item.type === "batch"}
              <div
                class="flex size-5 items-center justify-center rounded bg-violet-500/10"
              >
                <Package class="size-3 text-violet-500" />
              </div>
            {:else if item.type === "monitor"}
              <div
                class="flex size-5 items-center justify-center rounded bg-rose-500/10"
              >
                <Monitor class="size-3 text-rose-500" />
              </div>
            {:else}
              <div
                class="flex size-5 items-center justify-center rounded bg-primary/10"
              >
            ...
            {#if item.meta}
              {#if item.type === "scrape" && item.meta.images > 0}
                <Badge
                  variant="secondary"
                  class="h-4 rounded px-1 text-[8px] font-bold"
              ...
              {#if item.type === "scrape" && item.meta.screenshot}
                <Badge
                  variant="secondary"
                  class="h-4 rounded px-1 text-[8px] font-bold">ss</Badge
                >
              {/if}
          ...
          <div
            class="mb-0.5 truncate pr-4 text-xs font-semibold text-foreground"
          >
            {item.title}
          </div>
          {#if item.type === "scrape" && item.url && item.url !== "Unknown"}
            ...
            {#if hostname}
              <div
                class="truncate text-[11px] font-medium text-muted-foreground/80"
              >
                {hostname}
              </div>
            {/if}
          ...
          {#if item.preview}
            <div
              class="mt-1 line-clamp-2 text-[11px] leading-relaxed text-muted-foreground/70"
            >
              {item.preview}
            </div>
          {/if}
...
<div class="flex flex-1 flex-col bg-background md:flex-row">
  ...
  <main class="w-full min-w-0 flex-1">
    <div class="mx-auto max-w-6xl space-y-8 p-6">
      <div
        class="flex flex-col justify-between gap-4 border-b pb-6 md:flex-row md:items-end"
      >
        <div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} class="w-full">
        <TabsList
          class="mb-6 grid w-full max-w-3xl grid-cols-3 gap-1 sm:grid-cols-6"
        >
