...
const ScrapeOptionsSchema = v.object({
  ...
  mode: v.optional(
    v.union([v.picklist(["smart", "static", "dynamic"]), v.string()]),
    "smart",
  ),
  ...
  image_format: v.optional(
    v.union([v.picklist(["url", "blob"]), v.string()]),
    "url",
  ),
...
const SearchOptionsSchema = v.object({
  ...
  category: v.optional(
    v.union([v.picklist(["general", "news", "code"]), v.string()]),
    "",
  ),
  rerank: v.optional(
    v.union([
...
async function fetchTomoshi(endpoint: string, method: string, body?: any) {
  if (!PRIVATE_TOMOSHI_BACKEND_URL) {
    throw new Error("PRIVATE_TOMOSHI_BACKEND_URL is not set");
  }

  // Normalize: strip trailing slash and any '/v1' suffix from base URL
  // so it works regardless of how the user configured it
  const baseUrl = PRIVATE_TOMOSHI_BACKEND_URL.replace(/\/\/+$/, "").replace(
    /\/v1$/,
    "",
  );
  ...
  console.log("[Tomoshibi] Fetching", method, url);
  if (body) console.log("[Tomoshibi] Body:", JSON.stringify(body));
  ...
  try {
    ...
    if (response.status === 204) {
      console.log("[Tomoshibi] Response 204 for", url);
      return { success: true };
    }
    ...
    console.log("[Tomoshibi] Response OK for", url);
    return result;
  } catch (err: any) {
    console.error("Tomoshibi API Error:", err);
    // Include more detail in the error for debugging
    const detail = `Backend URL: ${PRIVATE_TOMOSHI_BACKEND_URL} | Endpoint: ${endpoint} | Method: ${method}`;
...
export const scrapeUrl = form(ScrapeOptionsSchema, async (data) => {
  ...
  if (rawUrls) {
    ...
    if (urls.length === 0)
      throw error(400, "At least one URL is required in urls");
    if (urls.length > 10)
      throw error(
        400,
        "Too many URLs (max 10) for sync scrape — use Batch tab for up to 20 async",
      );
    for (const u of urls) {
      try {
...
export const crawlUrl = form(CrawlOptionsSchema, async (data) => {
  ...
  const result = await fetchTomoshi("/v1/crawl", "POST", payload);
  return result; // { id, url, maxDepth, limit, ... }
});

export const getCrawlStatus = query(v.string(), async (id) => {
  const res = await fetchTomoshi(`/v1/crawl/${id}`, "GET");
...
export const searchWeb = form(SearchOptionsSchema, async (data) => {
  ...
  const result = await fetchTomoshi("/v1/search", "POST", processedData);
...
export const mapSite = form(MapOptionsSchema, async (data) => {
  ...
  const result = await fetchTomoshi("/v1/map", "POST", payload);
  return result as {
    url: string;
    count: number;
...
export const createMonitor = form(MonitorOptionsSchema, async (data) => {
  ...
  if (payload.interval_seconds < 3600)
    throw error(400, "interval_seconds must be at least 3600");
  ...
  const result = await fetchTomoshi("/v1/monitor", "POST", payload);
  return result as {
    id: string;
    url: string;
...
export const getMonitorStatus = query(v.string(), async (id) => {
  const res = await fetchTomoshi(`/v1/monitor/${id}`, "GET");
  return res as {
    id: string;
    url: string;
    interval_seconds: number;
...
export const deleteMonitor = command(v.string(), async (id) => {
  const res = await fetchTomoshi(`/v1/monitor/${id}`, "DELETE");
  return res;
});
