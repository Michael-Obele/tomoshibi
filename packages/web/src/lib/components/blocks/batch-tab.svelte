<script lang="ts">
  ...
  import { batchScrape, getBatchStatus } from "$lib/remote/tomoshibi.remote";
  ...
  let progress = $derived(
    status
      ? Math.round(
          ((status.completed + status.failed) / Math.max(1, status.total)) *
            100,
        )
      : 0,
  );
  ...
  function copyTasks() {
    ...
    navigator.clipboard.writeText(
      status.tasks.map((t: any) => t.url).join("\n"),
    );
    copied = true;
    ...
    >
      <div class="flex flex-col gap-2">
        ...
        <p class="pl-1 text-[11px] text-muted-foreground">
          Async via Redis + Asynq. Returns a <span class="font-mono"
            >batch_id</span
          > — poll status below. For sync multi-URL (max 10, no Redis) use Scrape
          tab with advanced batch.
        </p>
      </div>
      <div class="flex justify-end">
        <Button
          type="submit"
          disabled={!!batchScrape.pending}
      ...
      {:else}
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          <Button
            ...
            onclick={copyTasks}
          >
            {#if copied}<Check class="mr-2 size-3" /> Copied{:else}<Copy
                class="mr-2 size-3"
              /> Copy URLs{/if}
          </Button>
          <Button
...
{:else}
  ...
  >
    ...
    <p class="mt-1 max-w-md text-center text-xs text-muted-foreground">
      Async — enqueues one Asynq task per URL. Poll <span class="font-mono"
        >GET /v1/batch/:id</span
      > for aggregated status. Needs Redis.
    </p>
  </div>
{/if}
