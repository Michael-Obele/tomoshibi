    ...
    <form
      {...createMonitor.enhance(async ({ submit }) => {
        ...
        try {
          ...
          monitorError =
            e.body?.message || e.message || "Monitor creation failed";
        }
      })}
      class="space-y-4"
    >
      ...
      <p class="pl-1 text-[11px] text-muted-foreground">
        Scrapes on interval, hashes markdown, fires signed <span
          class="font-mono">X-Tomoshibi-Signature</span
  ...
  <Card.Root>
    <Card.Header class="pb-3">
      <Card.Title class="flex flex-wrap items-center gap-2 text-base">
        ...
        <span
          class="max-w-48 truncate font-mono text-xs font-normal text-muted-foreground"
          >{monitorId}</span
        >
        <Button
          variant="ghost"
      ...
      <Card.Description class="flex flex-wrap items-center gap-2 text-xs">
        {#if monitorStatus}
          <span
            >Next check: <span class="font-mono text-foreground"
    ...
    <Card.Content class="space-y-4">
      {#if monitorStatus}
        <div class="grid gap-3 rounded-lg border bg-muted/30 p-4 text-xs">
          ...
          <div class="flex justify-between">
            <span class="text-muted-foreground">Interval</span><span
              class="font-mono">{monitorStatus.interval_seconds}s</span
            >
          </div>
          {#if monitorStatus.webhook_url}<div class="flex justify-between">
              <span class="text-muted-foreground">Webhook</span><span
                class="max-w-64 truncate font-mono"
                >{monitorStatus.webhook_url}</span
              >
            </div>{/if}
          <div class="flex justify-between">
            <span class="text-muted-foreground">Next Check</span><span
              class="font-mono">{monitorStatus.next_check}</span
            >
          </div>
          {#if monitorStatus.last_hash}<div class="flex justify-between">
              <span class="text-muted-foreground">Last Hash</span><span
                class="max-w-64 truncate font-mono text-[11px]"
                >{monitorStatus.last_hash}</span
              >
            </div>{/if}
        </div>
...
{:else}
  ...
  >
    ...
    <p class="mt-1 max-w-md text-center text-xs text-muted-foreground">
      Create a monitor to hash markdown on interval and fire a signed webhook
      when content changes. Baseline recorded on first check.
    </p>
  </div>
{/if}
