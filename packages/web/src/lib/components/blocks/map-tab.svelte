<script lang="ts">
  ...
  import { mapSite } from "$lib/remote/tomoshibi.remote";
    ...
    >
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex flex-wrap gap-2 pt-1">
          ...
          <div class="flex items-center gap-2">
            <Badge variant="outline" class="h-6 text-[10px] uppercase"
              >Limit</Badge
            >
            <Input
              type="number"
...
{#if pending}
  ...
  >
    ...
    <p class="text-xs text-muted-foreground">
      Checking sitemap.xml, robots.txt and page links
    </p>
  </div>
{:else if mapError}
  <div
          ...
          {#each links.slice(0, 100) as link (link.url)}
            ...
            >
              <div class="min-w-0 flex-1">
                <div
                  class="truncate text-xs font-medium group-hover:text-primary"
                >
                  {link.title || link.url}
                </div>
                <div
...
{:else}
  ...
  >
    ...
    <p class="mt-1 max-w-md text-center text-xs text-muted-foreground">
      Discovers URLs via sitemap.xml → robots.txt → link traversal. Filter with
      search, cap with limit (max 5000).
    </p>
  </div>
{/if}
