<script lang="ts">
  ...
  import { searchWeb } from "$lib/remote/tomoshibi.remote";
...
>
        ...
        <div class="flex gap-2">
          ...
          {#if searchOptions.current.category}
            <input
              type="hidden"
              name="category"
              value={searchOptions.current.category}
            />
          {/if}
          ...
          <input
            type="hidden"
            name="limit"
            value={searchOptions.current.limit}
          />
          <input
            type="hidden"
            name="offset"
            value={searchOptions.current.offset}
          />
          <input
            type="hidden"
            name="maxAge"
            value={searchOptions.current.maxAge}
          />
  ...
  >
    <div class="flex items-center gap-4">
      ...
      {#if searchOptions.current.rerank}
        <span class="flex items-center gap-1.5"
          ><Sparkles class="size-3" /> TF-IDF rerank</span
        >
      {/if}
    </div>
    ...
    {#each (displayedSearchResult as any[]) || [] as item (item.url)}
      ...
      >
        <div>
          ...
          {/if}
          <p
            class="mb-3 line-clamp-3 text-xs leading-relaxed text-muted-foreground"
          >
          ...
          {#if item.relevance !== undefined}
            <div class="mb-2 flex items-center gap-1.5">
              <Badge
                variant="outline"
                class="h-5 rounded px-1.5 font-mono text-[9px]"
        ...
        <div class="mt-auto flex items-center justify-between border-t pt-3">
          <span
            class="max-w-30 truncate font-mono text-[10px] text-muted-foreground"
