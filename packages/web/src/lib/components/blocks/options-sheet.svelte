    ...
    <div class="grid gap-6 py-6">
      {#if type === "scrape"}
        <div class="grid grid-cols-4 items-center gap-4">
          ...
          <div
            class="col-span-3 flex shrink-0 items-center justify-start gap-1"
          >
            <Button
              size="sm"
        ...
        <div class="space-y-4">
          ...
          {#if options.images}
            <div class="grid grid-cols-4 items-center gap-4">
              ...
              >
                <Button
                  size="sm"
                  variant={options.image_format === "url"
                    ? "default"
                    : "outline"}
                ...
                <Button
                  size="sm"
                  variant={options.image_format === "blob"
                    ? "default"
                    : "outline"}
          ...
          <div class="grid grid-cols-4 items-center gap-4">
            ...
            <div class="col-span-3">
              <Switch bind:checked={options.summary} />
            </div>
          </div>
          {#if options.summary}
            <div class="grid grid-cols-4 items-center gap-4">
          ...
          <p class="pl-2 text-[11px] text-muted-foreground">
            When filled, <span class="font-mono">urls[]</span> is sent (max 10, exclusive
            with single URL) — sync, no Redis, limit 5 parallel. Leave empty for single-URL
            scrape.
          </p>
        </div>
      {/if}
      ...
      {#if type === "crawl"}
        <div class="space-y-4">
          ...
          {#if options.mode === "dynamic"}
            ...
            >
              Dynamic mode requires Chrome/Chromium on the backend server. If
              unavailable, scraping will fail.
            </div>
          {/if}
          <div class="grid grid-cols-4 items-center gap-4">
          ...
          <div class="grid grid-cols-4 items-center gap-4">
            ...
            <Input
              ...
              class="col-span-3 font-mono text-xs"
              placeholder="https://myapp.example.com/hooks/tomoshibi"
            />
          </div>
          <div class="grid grid-cols-4 items-center gap-4">
      ...
      {#if type === "map"}
        <div class="space-y-4">
          ...
          <p class="pl-2 text-[11px] text-muted-foreground">
            Map uses sitemap.xml → robots.txt → link fallback. No Redis
            required.
          </p>
        </div>
      {/if}
      ...
      {#if type === "monitor"}
        <div class="space-y-4">
          ...
          <p class="pl-2 text-[11px] text-muted-foreground">
            Hashes markdown on interval, fires signed webhook on change. Min
            3600s. Requires Redis.
          </p>
        </div>
      {/if}
      ...
      {#if type === "search"}
        ...
        <div class="grid grid-cols-4 items-center gap-4">
          ...
          <div
            class="col-span-3 flex shrink-0 flex-wrap items-center justify-start gap-1"
          >
            <Button
              size="sm"
        ...
        <p class="pl-2 text-[11px] text-muted-foreground">
          Category maps to SearXNG <span class="font-mono">categories</span>
          (general/news/it) + Brave <span class="font-mono">search_type</span>.
          Validated enum.
        </p>
        <div class="grid grid-cols-4 items-center gap-4">
        ...
        <p class="pl-2 text-[11px] text-muted-foreground">
          Lightweight pure-Go rerank (no ONNX) — reorders by query-term TF-IDF.
          Opt-in.
        </p>
        <div class="grid grid-cols-4 items-center gap-4">
