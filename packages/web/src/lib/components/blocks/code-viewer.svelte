<script lang="ts">
  ...
  function copyToClipboard() {
    ...
    if (activeTab === "markdown") {
      ...
      content = isMulti
        ? result.results.map((r: any) => r.html || "").join("\n\n")
        : result.html;
  ...
  >
    ...
    >
      <Tabs.List
        ...
        style="grid-template-columns: repeat({tabCount}, auto)"
      >
    ...
    <div class="relative min-h-0 flex-1">
      <ScrollArea class="h-144 p-4">
        {#if isMulti}
          <div class="mb-3 flex flex-wrap items-center gap-2">
            <Badge variant="secondary" class="text-[10px]"
              >{result.results.length} URLs</Badge
            >
              ...
              {#each result.results as item (item.url)}
                <div class="rounded-lg border bg-background p-3">
                  <div class="mb-2 flex items-center gap-2">
                    <Badge
                      variant={item.error ? "destructive" : "outline"}
                      class="text-[10px]">{item.url}</Badge
                    >
                    {#if item.word_count}<span
                        class="text-[10px] text-muted-foreground"
                        >{item.word_count} words</span
                      >{/if}
                    {#if item.error}<span class="text-xs text-destructive"
                        >{item.error}</span
                      >{/if}
                  </div>
                  <pre
        ...
        <Tabs.Content value="html" class="mt-0">
          ...
          {:else}
            <pre
              class="font-mono text-sm whitespace-pre-wrap text-foreground/80">{result.html ||
                "No HTML content"}</pre>
          {/if}
        </Tabs.Content>
        <Tabs.Content value="json" class="mt-0">
          <pre
            class="font-mono text-sm whitespace-pre-wrap text-foreground/80">{JSON.stringify(
              result,
              null,
              2,
            )}</pre>
        ...
        <Tabs.Content value="links" class="mt-0">
          {#if isMulti}
            {@const allLinks = result.results.flatMap(
              (r: any) => r.links || [],
            )}
            <pre
              class="font-mono text-sm whitespace-pre-wrap text-foreground/80">{formatLinks(
                allLinks,
              ) || "No links found"}</pre>
          {:else}
            <pre
            ...
            <div>
              <div class="mb-3 flex flex-wrap items-center gap-2">
                {#if result.screenshot.width && result.screenshot.height}
                  <Badge variant="outline" class="text-[10px]"
                    >{result.screenshot.width}×{result.screenshot
                      .height}px</Badge
                  >
                {/if}
                ...
                {#if result.screenshot.full_page}
                  ...
                  >
                {/if}
                {#if result.screenshot.truncated}
                  <Badge
                    variant="destructive"
                    class="text-[10px]"
                    title="Page taller than the server's screenshot height cap — capture covers the top of the page"
                    >Truncated</Badge
                  >
                {/if}
              </div>
