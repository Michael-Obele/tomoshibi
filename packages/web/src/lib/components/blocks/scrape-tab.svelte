<script lang="ts">
  ...
  import { scrapeUrl } from "$lib/remote/tomoshibi.remote";
    ...
    <form
      {...scrapeUrl.enhance(async ({ submit }) => {
        ...
        try {
            ...
            if (r.results && Array.isArray(r.results)) {
              ...
              addToHistory({
                ...
                meta: {
                  ...
                  images: r.results.reduce(
                    (a: number, x: any) => a + (x.images?.length || 0),
                    0,
                  ),
                  links: r.results.reduce(
                    (a: number, x: any) => a + (x.links?.length || 0),
                    0,
                  ),
                },
              ...
              const preview =
                ...
                r.markdown?.slice(0, 100).replace(/\n/g, " ") ||
                "";
              addToHistory({
                id: crypto.randomUUID(),
                type: "scrape",
          ...
          scrapeError =
            e.body?.message || e.message || "An unexpected error occurred";
        }
      })}
      class="space-y-4"
    >
      ...
      <div class="flex flex-col gap-2">
        <div class="flex gap-2">
          ...
          <input
            type="hidden"
            name="max_images"
            value={scrapeOptions.current.max_images || 10}
          />
          <input
            type="hidden"
          ...
          {#if scrapeOptions.current.actions}
            <input
              type="hidden"
              name="actions"
              value={scrapeOptions.current.actions}
            />
          {/if}
          {#if scrapeOptions.current.urls}
            <input
              type="hidden"
              name="urls"
              value={scrapeOptions.current.urls}
            />
          {/if}
  ...
  >
    <div class="flex flex-wrap items-center gap-2">
      <span class="flex items-center gap-1.5"
        ><Badge
          variant="outline"
          class="h-5 rounded px-1.5 text-[9px] uppercase">Format</Badge
        > Markdown</span
      >
      <span class="flex items-center gap-1.5"
        ><Badge
          variant="outline"
          class="h-5 rounded px-1.5 text-[9px] uppercase">Links</Badge
        >
      ...
      {#if scrapeOptions.current.urls}
        <Badge
          variant="secondary"
          class="h-5 rounded px-1.5 text-[9px] uppercase"
          >Sync batch {scrapeOptions.current.urls
            .split(/[\n,]+/)
...
{:else if displayedScrapeResult}
  <div
    class="grid animate-in grid-cols-1 gap-6 duration-700 fade-in lg:grid-cols-3"
  >
