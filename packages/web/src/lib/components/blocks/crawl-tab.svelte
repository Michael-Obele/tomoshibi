<script lang="ts">
  ...
  import { crawlUrl, getCrawlStatus } from "$lib/remote/tomoshibi.remote";
  ...
  let crawlList = $derived(
    crawlCurrent?.crawl?.pages ||
      ...
      (Array.isArray(crawlCurrent?.parsedResult)
        ? crawlCurrent?.parsedResult
        : []),
  );
  ...
  function getPageTitle(page: any) {
    ...
    const title =
      typeof page === "string" ? null : page?.metadata?.title || page?.title;
...
<section
  class="overflow-hidden rounded-xl border bg-card/50 shadow-sm backdrop-blur-sm"
>
  <div class="p-6">
    <form
      ...
      <div class="flex flex-col gap-2">
        <div class="flex gap-2">
          ...
          <input
            type="hidden"
            name="mode"
            value={crawlOptions.current.mode || "smart"}
          />

          <input
            type="hidden"
            name="maxDepth"
            value={crawlOptions.current.maxDepth || 2}
          />
          <input
            type="hidden"
            name="limit"
            value={crawlOptions.current.limit || 10}
          />
          ...
          <input
            type="hidden"
            name="image_format"
            value={crawlOptions.current.image_format || "url"}
          />
          {#if crawlOptions.current.include_paths}
            <input
              type="hidden"
              name="include_paths"
              value={crawlOptions.current.include_paths}
            />
          {/if}
          {#if crawlOptions.current.exclude_paths}
            <input
              type="hidden"
              name="exclude_paths"
              value={crawlOptions.current.exclude_paths}
            />
          {/if}
          {#if crawlOptions.current.webhook_url}
            <input
              type="hidden"
              name="webhook_url"
              value={crawlOptions.current.webhook_url}
            />
          {/if}
          {#if crawlOptions.current.webhook_secret}
            <input
              type="hidden"
              name="webhook_secret"
              value={crawlOptions.current.webhook_secret}
            />
          {/if}
        ...
        {#each crawlUrl.fields.url?.issues() || [] as issue (issue.message)}
          <p
            class="mt-1 flex items-center gap-1.5 pl-1 text-xs text-destructive"
          >
  ...
  >
    <div class="flex items-center gap-4">
      ...
      <span class="flex items-center gap-1.5"
        >Limit: {crawlOptions.current.limit || 10} pages</span
      >
    </div>
...
{#if crawlId}
  <div class="animate-in space-y-6 duration-500 fade-in">
    ...
    >
      ...
      <div
        class="absolute -top-24 -right-24 h-48 w-48 rounded-full bg-primary/5 blur-[80px]"
      ></div>

      <div
        class="relative mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"
      >
        <div class="flex items-center gap-4">
          <div
          ...
          >
            <div
              class="absolute inset-0 rounded-xl bg-primary/10 {crawlCurrent?.state ===
              'active'
                ? 'animate-pulse'
                : ''}"
            ></div>
      ...
      <div class="space-y-4">
        ...
        <div
          class="h-2 w-full overflow-hidden rounded-full border bg-muted/50 p-0.5"
        >
          <div
            class={cn(
        ...
        <p
          class="pb-2 text-[10px] font-medium tracking-wide text-muted-foreground"
        >
          ...
          >
            ...
            <div class="relative z-10 mb-4">
              <div class="mb-3 flex items-start justify-between gap-4">
                ...
                >
                  <ExternalLink
                    class="size-3.5 text-muted-foreground hover:text-primary"
                  />
                </Button>
              </div>
            ...
            <div class="relative z-10 mt-auto border-t border-border/40 pt-3">
              <div
                class="flex items-center gap-1.5 truncate font-mono text-[11px]"
              >
    ...
    {:else if crawlFailed && crawlFailedUrls.length > 0}
      <div class="animate-in space-y-3 duration-500 fade-in">
        ...
        >
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              {crawlFailedUrls.length} URL{crawlFailedUrls.length > 1
                ? "s"
                : ""} failed to crawl.
            </p>
          </div>
        ...
        {#each crawlFailedUrls as failedUrl (failedUrl.url)}
          <div
            class="rounded-lg border border-destructive/10 bg-destructive/5 p-4"
          >
            <p class="mb-1 truncate font-mono text-xs font-medium">
              {failedUrl.url}
            </p>
...
{:else}
  ...
  >
    ...
    <p class="text-center text-lg font-medium tracking-tight">
      Define a boundary to start a <span class="text-primary italic"
        >massive</span
      > discovery mission.
    </p>
